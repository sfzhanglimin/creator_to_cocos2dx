...
class Node
{
    ...
    parse_node_properties()
    {
        ...
        this.convertHightVersionToLowVersion(data);
    ...
    /**将fire文件里面高版本数据向低版本兼容 ,目前为2.3 to 1.9*/
    convertHightVersionToLowVersion(aJson)
    {
        //add by zlm to fix 2.3
        ...
        if (aJson._trs && aJson._trs.array)
        {
            ...
            aJson._position = { x: aJson._trs.array[vx], y: aJson._trs.array[vy] }


            vx = 7;
            vy = 8;
            aJson._scaleX = aJson._trs.array[vx]
            aJson._scaleY = aJson._trs.array[vy]
        }

        //2.3里面_eulerAngles为rotation
        if (aJson._eulerAngles)
        {
            let key = "rotationSkewX";
            aJson._rotationX = aJson._eulerAngles.x
            key = "rotationSkewY";
            aJson._rotationY = aJson._eulerAngles.y
        }
